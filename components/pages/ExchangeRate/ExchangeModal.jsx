...
export default function ExchangeModal({ data, isOpen, onClose, onSubmit }) {
  ...
  const isValid = currency !== "" && rateCount !== "";
  const rateOptions = data?.CountryList.map(item => item.CurrencyCode);
  const current = data?.CountryList.find(item => item.CurrencyCode === currency);
